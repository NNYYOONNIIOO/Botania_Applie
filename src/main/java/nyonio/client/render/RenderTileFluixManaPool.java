...
public class RenderTileFluixManaPool extends TileEntitySpecialRenderer<TileFluixManaPool> {
    
    @Override
    public void render(@Nonnull TileFluixManaPool pool, double d0, double d1, double d2, float f, int digProgress, float unused) {
        if(pool != null && (!pool.getWorld().isBlockLoaded(pool.getPos(), false)
                || !(pool.getWorld().getBlockState(pool.getPos()).getBlock() instanceof BlockFluixManaPool)))
            return;
        ...
        boolean fab = pool.getWorld().getBlockState(pool.getPos()).getValue(BotaniaStateProps.POOL_VARIANT) == PoolVariant.FABULOUS;
